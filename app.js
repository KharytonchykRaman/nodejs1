function randomNum(min, max) {
  ...
  http
    .createServer(function (request, response) {
      ...
      products.map((pr) => response.write(`На ${pr.name} скидка ${pr.discount}\n`));
    ...
    response.write(`Пользователь с наибольшим числом друзей : ${getUserWithMostFriends([user1,user2,user3,user4]).name}\n`);
